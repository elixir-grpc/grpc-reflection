  ...
  for version <- versions do
    describe version do
      ...
      test "should reflect all scalar field types on ScalarRequest", ctx do
        ...
        assert %{type: :TYPE_FIXED32, number: 9, label: :LABEL_OPTIONAL} =
                 by_name["fixed32_field"]

        assert %{type: :TYPE_FIXED64, number: 10, label: :LABEL_OPTIONAL} =
                 by_name["fixed64_field"]
        ...
        assert %{type: :TYPE_STRING, number: 100, label: :LABEL_OPTIONAL} =
                 by_name["sparse_field_1"]
      ...
      test "should reflect proto3 optional fields with presence tracking", ctx do
        ...
        assert %{type: :TYPE_STRING, number: 18, proto3_optional: true} =
                 by_name["optional_string"]
